...
internal sealed class WgcFps : IDisposable
{
    ...
    /// <paramref name="sharedDevice"/>: a device from <see cref="CreateSharedDevice"/>, reused across every
    /// meter in one SmartCapture run instead of each meter creating (and never releasing until RCW-GC) its
    /// own D3D11 device — a store launch with several new top-level windows to consider can otherwise stack
    /// up several live devices at once for no benefit, since one device can back many capture sessions.
    /// Omit to create a private device for this instance alone (unshared).</summary>
    ...
    /// <summary>Creates one D3D11 device for a caller to share across every <see cref="TryCreate"/> call in
    /// a single SmartCapture run (pass the result as <paramref name="sharedDevice"/> above).</summary>
