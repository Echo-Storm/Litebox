...
internal sealed class MainWindow : Form, IMessageFilter
{
    ...
    // Debounces the search box: ApplyFilter -> RebuildView -> MeasureContentFits re-scans every row of
    // the (possibly 15000-row) view to re-fit non-stretch columns, on every single keystroke otherwise -
    // wasted CPU + input latency on a library near that size. 150ms is short enough to feel instant once
    // typing pauses, long enough to collapse a fast typist's whole word into one measure pass.
