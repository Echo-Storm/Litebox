...
internal static class SmartCapture
{
    ...
    private static void Run(int rootPid, SmartCaptureConfig cfg, int displayMs, int maxMs, Action onReveal, int fadeMs = 0)
    {
        ...
        // useFps is off) instead of each meter creating its own; several candidate windows appearing close
        // together (a store launch, say) would otherwise stack up several live devices for no benefit.
        ...
        {
            foreach (var kv in meters) { try { kv.Value.meter.Dispose(); } catch { } }
            try { (sharedDevice as IDisposable)?.Dispose(); } catch { }
        }
