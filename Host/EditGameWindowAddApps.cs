...
internal sealed partial class EditGameWindow
{
    ...
    private void ReloadAddApps()
    {
        ...
        // Documents (Edit Game → Documents tab; Section=="Document") are additional-application records
        // too, but IsLikelyVersion has no concept of them (its rule predates that tab) and routes every
        // one into the "Apps" bucket below — exclude them here so they're managed exclusively by the
        // dedicated Documents page instead of ALSO being editable/deletable/launchable from this one.
